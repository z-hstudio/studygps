...
async function launch(t, options = {}) {
  ...
  const clerk = {
    ...
    mountSignIn(node, props) { node.textContent = 'TEST SIGN IN'; this.signInProps = props; },
    unmountSignIn(node) { node.replaceChildren(); },
    mountSignUp(node, props) { node.textContent = 'TEST SIGN UP'; this.signUpProps = props; },
    unmountSignUp(node) { node.replaceChildren(); },
    mountUserButton() {}, unmountUserButton() {},
  ...
  await tick();
  return {
    w, clerk, requests, document: w.document,
