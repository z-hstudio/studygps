...
function memoryRepository() {
  ...
  const reads = [];
  return {
    profiles, classrooms, enrollments, learning, advice, reads,
    ...
    async listStudents(teacherId) {
      ...
      return [...profiles.values()].filter((row) => classrooms.get(enrollments.get(row.id))?.teacherId === teacherId).map((row) => {
        ...
        return { id: row.id, name: row.name, email: row.email, goal: row.goal, overallScore: state?.plan.overall_score ?? null, completedTasks: state?.completedTaskIds.length ?? 0, totalTasks: state?.plan.priorities.length ?? 0, updatedAt: state?.updatedAt ?? null, focusTopic: state?.plan.priorities[0]?.topic_id ?? null };
      });
    },
