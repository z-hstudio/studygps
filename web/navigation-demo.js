(function () {
  ...
  function render() {
    ...
    const teacher = node('a', 'text-link', t.teacher); teacher.href = '/portal.html?lang=' + lang + '&demo=alex#student-detail'; teacher.id = 'lab-teacher-link'; links.append(teacher);
    const teacherNote = node('p', 'lab-provenance', t.teacherNote); teacherNote.id = 'lab-teacher-note'; teacher.setAttribute('aria-describedby', teacherNote.id);
    root.append(links, teacherNote);
  ...
  async function update() {
    ...
    try {
      const response = await fetch('/api/navigation-demo?' + new URLSearchParams({ ...settings, lang: locale() }), { signal: controller.signal });
  ...
  new MutationObserver(() => {
    // Scenario text is generated in the selected language, so refresh the route
    // too. Never re-render an old language's interpolated goals or feedback.
    data = null; render(); void update();
